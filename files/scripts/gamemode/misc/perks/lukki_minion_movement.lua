...
local targets2 = EntityGetInRadiusWithTag( x, y, 32, "homing_target" )
...
-- remove root entity from targets
for i = #targets2, 1, -1 do
	if targets2[i] == owner_id then
...
if ( area_damage_component ~= nil and owner_id ~= 0 ) then
	ComponentSetValue2( area_damage_component, "entity_responsible", owner_id )
end
...
if ( owner_id ~= 0 and EntityGetIsAlive(owner_id) ) then
	...
	-- if the owner is dead, kill the minion
