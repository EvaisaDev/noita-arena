...
function spawn(what, x, y, rand_x, rand_y)
	...
	if ( v ~= nil ) then
		return entity_load_camera_bound( v, x + x_offset, y + y_offset, rand_x, rand_y)
	end
end
...
function spawn2(what, x, y, rand_x, rand_y)
	...
	local v = random_from_table( what, x, y )
	if ( v ~= nil ) then
		return entity_load_camera_bound( v, x, y, rand_x, rand_y)
	end
end
...
function DEBUG_spawn_all( what, x, y, width )
	...
	for i,v in ipairs(what) do
		if( v.prob ~= nil and v.prob > 0 ) then
			local ent = entity_load_camera_bound( v, x, y, 0, 0)
			if( ent ~= nil ) then
...
function spawn_with_limited_random(what, x, y, rand_x, rand_y, entities_to_randomize)
	...
	if ( v ~= nil ) then
		...
		return entity_load_camera_bound( v, x + x_offset, y + y_offset, random_x, random_y )
	end
end
