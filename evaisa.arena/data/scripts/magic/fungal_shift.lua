...
function fungal_shift( entity, x, y, debug_no_limits )
	...
	while converted_any == false and convert_tries < 20 do
		...
		if material_blacklist_data ~= nil then
			local skip = material_blacklist_data[to.material]
			if not skip then
				for _, mat in ipairs(from.materials) do
					if material_blacklist_data[mat] then
						skip = true
						break
					end
				end
